...
lang = "D"
log = logging.getLogger("codeintel.d")
#log.setLevel(logging.DEBUG)


# These keywords are copied from "D-mainlex.udl" - be sure to keep both
# of them in sync.
keywords = [
    # Keywords
        "abstract",
...
# lexers. You must define a Lexer class for lang D. If Komodo's
# scintilla lexer for D is UDL-based, then this is simply:
#
#   from codeintel2.udl import UDLLexer
#   class DLexer(UDLLexer):
#       lang = lang
#
# Otherwise (the lexer for D is one of Komodo's existing C++ lexers
...
#           self._lexer = SilverCity.find_lexer_module_by_id(ScintillaConstants.SCLEX_D)
#           self._keyword_lists = [
...
# D content and returns a description of the code in that file.
# See "cile_d.py" for more details.
#
# The CILE Driver is a class that calls this CILE. If D is
...
class DCILEDriver(UDLCILEDriver):
    lang = lang

    def scan_purelang(self, buf):
        import cile_d
        return cile_d.scan_buf(buf)
...
def register(mgr):
    ...
    mgr.set_lang_info(
        lang,
        silvercity_lexer=DLexer(),
        buf_class=DBuffer,
        langintel_class=DLangIntel,
