...
"""A Code Intelligence Language Engine for the D language.
...
Module Usage:
    from cile_d import scan
    mtime = os.stat("bar.d")[stat.ST_MTIME]
    content = open("bar.d", "r").read()
    scan(content, "bar.d", mtime=mtime)
"""
...
def scan_buf(buf, mtime=None, lang="D"):
    """Scan the given DBuffer return an ElementTree (conforming
    to the CIX schema) giving a summary of its code elements.
    
    @param buf {DBuffer} is the D buffer to scan
    ...
    # Dev Note:
    # This is where you process the D content and add CIX elements
